...
text_for_weather = ['Всем удачного дня и хорошего настроения!',
                    'Добрейшего денека, вот вам характеристика сегодняшнего дня!',
                    'Сегодня в мире такая ситуация по погоде и курсам вылют',
                    'Сегодня мы хотим поделиться с вами важной информацией о погоде и курсе валют',
                    'Давайте вместе посмотрим прогноз и узнаем, чего ожидать от предстоящего дня']
...
def send_weather_courses():
    ...
    bot.send_photo(CHANNEL_NAME, img, text_for_weather[random.randint(0, len(text_for_weather)-1)])
